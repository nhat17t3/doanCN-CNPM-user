...
function InforTutor(props) {
  ...
  useEffect(() => {
    dispatch(getListComment(tutorId));
  }, [a]);
  ...
  const handleInviteTutorClick = () => {
    const inviteTutorUrl = `/invite-tutor/${tutorId}`;
    history.push(inviteTutorUrl);
  };
                    ...
                    <div className="tutor-infor__item">
                      {disablebtn === false ? (
                        <button
                          ...
                          onClick={handleInviteTutorClick}
                        >
                          Mời dạy
                        </button>
                      ) : (
