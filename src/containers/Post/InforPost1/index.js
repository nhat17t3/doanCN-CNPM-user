...
import { getListPost, getListPostByToken } from "../../../actions";
import {
  addSuggestion,
  getListSuggestion,
} from "../../../actions/Suggestion/suggestion.actions";
import Layout from "../../../components/Layout";
import ListTutorApply from "../ListTutorApply";
...
function InforPost1(props) {
  ...
  const postView = useSelector((state) => {
    const foundPost = state.posts.listPostByToken?.find(
      (x) => x.id === +postId
    );
    return foundPost;
  });
  ...
  useEffect(() => {
    if (
      listSuggestion.find((x) => x.idPost === +postId && x.idTutor === +userID)
    )
      setDisablebtn(true);
  }, [listSuggestion]);
                ...
                <div className="post-infor__right">
                  ...
                  <div className="post-infor__right-bottom">
                    ...
                    <div className="tutor-infor__comment">
                      <ListTutorApply listTutorApply={postView?.candidates} postId={postId} statusOpen = {postView?.status} />
                    </div>
                  </div>
                </div>
              </div>
