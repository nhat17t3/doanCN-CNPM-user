...
import { getListPostByToken } from "../Post/post.actions";
...
export const addInvitationTutor = (tutorId,post) => {
  return async (dispatch) => {
    dispatch({
      type: invitationConstants.ADD_INVITATION_REQUEST,
    });
    const res = await axios.post(`/api/invite?idTutor=${tutorId}&idPost=${post.id}`);
