...
export const getListComment = (tutorId) => {
  return async (dispatch) => {
    dispatch({ type: commentConstants.GET_ALL_COMMENT_REQUEST });
    const res = await axios.get(`/api/comment/${tutorId}`);
...
export const addRate = (form) => {
  return async (dispatch) => {
    dispatch({
      type: commentConstants.ADD_RATE_REQUEST,
    });
    const res = await axios.post(`/api/rate`, form);

    if (res.status === 200) {
      const { message } = res.data;
      dispatch({
        type: commentConstants.ADD_RATE_SUCCESS,
        payload: { message },
      });
      ...
      if (res.status === 400) {
        dispatch({
          type: commentConstants.ADD_RATE_FAILURE,
          payload: { error: res.data.error },
        });
