...
import { getListPost, getListPostByToken } from "../Post/post.actions";
...
export const addSuggestion = (postId) => {
  return async (dispatch) => {
    dispatch({
      type: suggestionConstants.ADD_SUGGESTION_REQUEST,
    });
    const res = await axios.put(`/api/post/apply?idPost=${postId}`);
...
export const cancelAddSuggestion = (postID) => {
  return async (dispatch) => {
    dispatch({
      type: suggestionConstants.CANCEL_ADD_SUGGESTION_REQUEST,
    });
    const res = await axios.put(`/api/post/cancelApply?idPost=${postID}`);

    if (res.status === 200) {
      const { message } = res.data;
      dispatch({
        type: suggestionConstants.CANCEL_ADD_SUGGESTION_SUCCESS,
        payload: { message },
      });
      toast.success("Lời đề nghị dạy đã được hủy");
      ...
      if (res.status === 400) {
        dispatch({
          type: suggestionConstants.CANCEL_ADD_SUGGESTION_FAILURE,
          payload: { error: res.data.error },
        });
      toast.error(" Hủy lời đề nghị dạy đã xảy ra lỗi");
...
export const acceptSuggestion = (postId,tutorId) => {
  return async (dispatch) => {
    // const id = form.get("id");
    dispatch({ type: suggestionConstants.ACCEPT_SUGGESTION_REQUEST });
    const res = await axios.put(`/api/post/accept?postId=${postId}&tutorId=${tutorId}`);
